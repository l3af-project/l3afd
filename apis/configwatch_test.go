...
func TestMatchHostnamesWithRegexp(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    args{pattern: "l3afd-us.l3af.io", host: "l3afd-lfn.l3af.io"},
			want:    false,
			wantErr: false,
		},
		{
...
func TestMatchHostnames(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "PatternMatchCheck",
			args:    args{pattern: "*.l3af.io", host: "l3afd-lfn.l3af.io"},
			want:    true,
			wantErr: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := matchHostnames(tt.args.pattern, tt.args.host)
			if !reflect.DeepEqual(got, tt.want) {
