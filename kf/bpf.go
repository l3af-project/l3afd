...
func (b *BPF) Start(ifaceName, direction string, chain bool) error {
	...
	if !b.Program.UserProgramDaemon {
		...
		if err := b.VerifyPinnedMapExists(chain); err != nil {
			return fmt.Errorf("no userprogram and failed to find pinned file %s, %v", b.Program.MapName, err)
		}
		return nil
	}
	...
	if err := b.VerifyPinnedMapExists(chain); err != nil {
		return fmt.Errorf("failed to find pinned file %s  %v", b.Program.MapName, err)
	}
...
func (b *BPF) VerifyPinnedMapExists(chain bool) error {
	...
	if len(b.Program.MapName) > 0 {
		...
		if err != nil {
			err = fmt.Errorf("failed to find pinned file %s err %v", b.Program.MapName, err)
...
func (b *BPF) VerifyPinnedMapVanish(chain bool) error {
	...
	err = fmt.Errorf("%s map file was never removed by BPF program %s err %v", b.Program.MapName, b.Program.Name, err)
	log.Error().Err(err).Msg("")
	return err
}
