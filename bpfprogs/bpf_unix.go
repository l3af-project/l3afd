...
func (b *BPF) UnloadTCProgram(ifaceName, direction string) error {
	...
	for _, qdisc := range qdiscs {
		...
		if iface.Name == ifaceName {
			...
			case "ingress":
				...
				ingressHandle = qdisc.Msg.Handle
			default:
				log.Info().Msgf("qdisc kind for %s : %s", ifaceName, qdisc.Kind)
			}
		}
	}
	...
	if clsactFound && !ingressFound && !htbFound {
		...
		if len(tcfilts) > 0 {
			...
			log.Warn().Msgf("clasct filters not found for interface \"%s\" direction %s", ifaceName, direction)
		}
	} else if !clsactFound && ingressFound && htbFound {
		...
		if len(tcfilts) > 0 {
			...
			log.Warn().Msgf("ingress or htp filter not found for interface %s direction %s", ifaceName, direction)
		}
	}
